    ...
    def RUN(self):
        ...
        for MODEL in MODELS:
            ...
            for ind, GROUP in GROUPS.iterrows():
                ...
                sbatch_cmd = "sbatch "+'slurm-r1-rusty.sh'
        ...
        try:
            ...
            for MODEL in MODELS:
                ...
                for ind, GROUP in GROUPS.iterrows():
                    ...
                    current_directory = os.getcwd()
                    os.chdir(task_path)
                    sbatch_cmd = "sbatch -p ccb -J %s -t 125 disBatch %s"%(MODEL,task_file_name)
...
class CONSENSUS_MODE_ROUND_2:
    ...
    def RUN_CONSENSUS(self):
        ...
        for ind, GROUP in GROUPS.iterrows():
            ...
            os.chdir(task_path)
            sbatch_cmd = "sbatch -p ccb -J %s -t 125 disBatch %s"%(consensus_MODEL_name,task_file_name)
            # print(sbatch_cmd)
            subprocess.run(sbatch_cmd,shell=True)
...
class ANALYSIS_SUIT:
    ...
    def collect_r2_output(self):
        ...
        for MODEL in MODELS:
            ...
            a_model_path = os.path.join(self.output_path,MODEL)
            for ind, GROUP in GROUPS.iterrows():
                sorted_output_file_path = os.path.join(a_model_path,"round2",GROUP['group'],"outputs","OutPut_SORTED_%s-%s"%(MODEL,GROUP['group']))
    ...
    def make_matrix_log(self):
        ...
        cv_profile_path = input("Please provide the path to your CV profile (etc. MODEL CV_1 ... CV_n)\nThe first line will be read as headers\n")
        df_cv_profile=pd.read_csv(cv_profile_path,header=0,delim_whitespace=True,comment='#')
        cv_choice=input("\n==========Your available Collective Variables (CV):\n%s\n\nPlease select the CV that you want to be SORTED:\n"%(df_cv_profile.columns.to_list()))
        df_cv_profile_sorted=df_cv_profile.sort_values(by=[cv_choice],ignore_index=True)
        df_cv_profile_sorted=df_cv_profile_sorted.rename(columns={df_cv_profile_sorted.columns[0]: "model"})
        col = df_cv_profile_sorted.pop(cv_choice)
        df_cv_profile_sorted.insert(1, col.name, col)
        # print(df_cv_profile)
        # print(df_cv_profile_sorted)
        collected_r2_path = os.path.join(self.output_path,"0-COLLECTED_R2_OUTPUTS")
        df_cv_profile_sorted.to_csv(collected_r2_path+"/SORTED_ON_%s"%(cv_choice),sep='\t')
        GROUPS = pd.read_csv(self.group_list,names=['particle_file','group','nframe'],
                                        delim_whitespace='True',comment='#')
        ...
        for idx,MODEL in df_cv_profile_sorted.iterrows():
            for ind, GROUP in GROUPS.iterrows():
                # print(GROUP)
                matrix_element=[]
                copied_output_path=os.path.join(collected_r2_path,"OutPut_SORTED_%s-%s"%(MODEL['model'],GROUP['group']))
        ...
        matrix_model=matrix_model.astype(float)        #CHANGE OF TYPE MUST BE PRIOR TO ANY FUNCTION
        ...
        matrix_model_norm.to_csv(bife_output_filename+"-annotated",sep='\t',float_format='%1.6f')
    ...
    def execute_cryoBIFE(self):
        ...
        u =fit.summary()
        bife_output_filename=os.path.join(bife_workdir_path,"cryoBIFE_%s.out"%(CVchoice))
